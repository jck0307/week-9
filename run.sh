...
# 定义各目录
output_dir=./out # 训练目录
dataset_dir=./my_computer/quiz-w8-doc # 数据集目录，这里是写死的，记得修改
...
for i in {0..0}  # ..4for循环中的代码执行5此，这里的左右边界都包含，也就是一共训练500个step，每100step验证一次
do
    echo "############" $i "runnning #################"
    last=$[$i*100]
    current=$[($i+1)*100 + 1]
...
# 在test.jpg上验证导出的模型
python ./my_computer/quiz-w8-doc/inference.py --output_dir=$output_dir --dataset_dir=$dataset_dir
